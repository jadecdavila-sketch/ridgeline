...
export const beacon: ScorecardData = {
  ...
  dualReads: [
    {
      ...
      val: 'B asset',
      note: 'Attractive imaging market and real scale, but out of lane, capital-intensive, and scored on partial data. The capex changes the cash math.',
    },
    {
      side: 'p',
      label: 'Price discipline',
      val: 'Fair on EBITDA',
      note: '9.0x looks fair, but capex intensity means EBITDA overstates free cash. Price the free cash, not the headline multiple.',
    },
  ],
  reasonsLead: 'Selective.',
  reasonsBody:
    'An attractive imaging market with a credible network, but three things hold it to a 5. Diagnostic imaging is out of lane, so the bar rises. It is capital-intensive, which means EBITDA flatters the cash story and the entry should be read against free cash, not the headline multiple. And the data loaded was partial, so the read carries a confidence caveat. No gate trips; the call is the lane plus the capex math, and it needs the missing detail before it firms up.',
  ...
  axes: [
    {
      ...
      read: [
        ...
        ', so the bar rises, and it is more equipment- and facility-intensive than the core services lanes. Recurring referral-driven volume, but a different capital profile.',
      ],
      evidence: [
        {
          id: 'e-lane',
    ...
    },
    {
      ...
      ai: 'Non-negotiable',
      read: [
        'Imaging volume is referral-driven and partly schedulable; some studies defer in a downturn. Recurring enough to grade, but the equipment base means a fixed-cost drag if volume softens.',
      ],
      chipCls: 'mon',
      chipLabel: 'Mixed',
      barPct: 62,
    ...
    },
    {
      ...
      read: [
        'Graded a B asset where the ',
        { cite: 'e-qp', tag: 'DATA', label: 'capex intensity changes the price math' },
        '. A 9.0x EBITDA entry looks fair, but maintenance capex pulls free cash well below EBITDA, so the effective multiple on cash is higher than it reads.',
      ],
      evidence: [
        {
          id: 'e-qp',
          ...
          snip: [
            ...
            '; FCF conversion is the number to underwrite, not EBITDA. The headline multiple understates the price on cash.',
          ],
    ...
    },
    {
      ...
      read: [
        'Credible levers: modality-mix shift toward higher-margin studies, throughput and scheduling optimization, and de novo or tuck-in centers, though de novo is capital-heavy. The operating levers are real; the growth levers carry a capital cost.',
      ],
      chipCls: 'strong',
      chipLabel: 'Credible',
      barPct: 70,
    },
    {
      ...
      read: [
        'Capable operator, but the read is on ',
  ...
  moatNote: ['Matched on sub-sector, size, and entry multiple. The imaging set is thin and capital-intensive, which is itself part of the read.'],
  comps: [
    ...
    { name: 'Summit Imaging Partners', year: '2018', mult: '8.9x', tag: 'held', tagLabel: 'Tracking', moic: '—', status: 'Active' },
  ...
  moatEvidence: {
    ...
    snip: [
      ...
      { hl: '1.6x' },
      '. Beacon at 9.0x is fairly priced on EBITDA, but the FCF read is what decides it.',
    ],
  ...
  chat: {
    ...
    items: [
      {
        q: 'Why a 5?',
        a: 'Three drags. Diagnostic imaging is out of lane, so the bar rises. It is capital-intensive, so EBITDA overstates free cash and the real price on cash is higher than 9.0x suggests. And the data loaded was partial, so the read is provisional. None of that is disqualifying, which is why it is selective, not a pass, but the lane plus the capex math plus the missing detail keep it at a 5.<div class="src-tags"><span class="qtag cim">CIM</span><span class="qtag data">DATA</span></div>',
      },
      {
        q: 'Why the partial-data caveat?',
        a: 'The maintenance-versus-growth capex split, modality mix, and equipment age were not fully provided, and those are exactly the numbers that decide an imaging deal. Without them the free-cash picture is an estimate, so I have flagged the read as preliminary. It is the kind of gap that can move the score in either direction once the data room is complete.<div class="src-tags"><span class="qtag qoe">QoE</span></div>',
      },
      {
        q: 'How does the capex change it?',
        a: 'Imaging carries a heavy equipment base, so maintenance capex pulls free cash well below EBITDA, more than a services roll-up. A 9.0x EBITDA entry can be a meaningfully higher multiple on cash. The discipline is to underwrite FCF conversion and read the entry against free cash, not the headline. The firm’s imaging returns run below the services median for exactly this reason.<div class="src-tags"><span class="qtag data">DATA</span><span class="qtag chron">CHRON</span></div>',
      },
      {
        q: 'Which of our deals does this look like?',
        a: 'Imaging names. Clearview returned 2.3x at 8.6x and Radius is tracking 1.9x at 9.2x; Apex, entered above 10x, returned 1.6x. The set returns below the services-roll-up median, which is the capex tax showing up in the record. Beacon at 9.0x is fairly priced on EBITDA, but the comps say underwrite the cash, not the multiple.<div class="src-tags"><span class="qtag chron">CHRON</span></div>',
      },
    ],
  },
}
